...
//                    mInterstitialAd = null
    ...
    fun logEventFirebase(eventName: String, paramName: String, value: String) {
        ...
        Event.buildWithEventName(eventName).setCustomStringValue(paramName, value).send()
    }
    ...
    fun showNativeAdsBottom(frameAdsNative: FrameLayout, uuidAds: String) {
        ...
        val adLoader = builder.withAdListener(object : AdListener() {
            override fun onAdFailedToLoad(loadAdError: LoadAdError) {
                ...
                setLogDataToFirebase(
                    AdsLogModel(
                        adsId = uuidAds, adsName = "Ads Native Theme", message = loadAdError.message,
                        deviceName = Common.getDeviceName(this@BaseActivity)
                    )
                )
            }
